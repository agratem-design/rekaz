  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ref={ref}
        className={cn(
          "relative z-[100000] pointer-events-auto max-h-[min(24rem,60vh)] min-w-[8rem] overflow-hidden rounded-md border bg-popover text-popover-foreground shadow-md data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
          position === "popper" &&
      ...
      >
        {searchable && (
          <div className="flex items-center border-b border-border px-2 py-1.5">
            ...
            <input
              ...
              className="flex h-7 w-full bg-transparent px-2 text-sm outline-none placeholder:text-muted-foreground"
              onKeyDown={(e) => {
        ...
        <SelectPrimitive.Viewport
          className={cn(
            "p-1 max-h-[min(280px,55vh)] overflow-y-auto",
            position === "popper" &&
              "w-full min-w-[var(--radix-select-trigger-width)]",
          )}
        >
          <SelectSearchContext.Provider value={search.toLowerCase()}>
            {children}
...
>(({ className, children, ...props }, ref) => {
  ...
  if (search) {
    ...
    // Also check the value prop for matching
    const matchesSearch = textContent.toLowerCase().includes(search) || 
      (props.value && props.value.toLowerCase().includes(search));
