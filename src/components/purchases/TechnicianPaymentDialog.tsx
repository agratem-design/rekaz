...
interface TechnicianPaymentDialogProps {
  open: boolean;
...
export function TechnicianPaymentDialog({ open, onOpenChange, technicianId, technicianName, purchases }: TechnicianPaymentDialogProps) {
  ...
  const payMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["technician-purchases"] });
      ...
      toast({ title: "تم التسديد", description: `تم تسديد ${formatCurrencyLYD(amount)} لـ ${technicianName}` });
      handleClose();
    },
    onError: () => {
  ...
  const previewAllocation = () => {
    const result: { invoice: string; project: string; amount: number }[] = [];
    let rem = amount;
    for (const p of unpaidPurchases) {
      if (rem <= 0) break;
      const owed = Number(p.total_amount) - Number(p.paid_amount);
      const alloc = Math.min(rem, owed);
      result.push({ invoice: p.invoice_number || p.id.slice(0, 8), project: p.project_name || "-", amount: alloc });
      rem -= alloc;
    }
    return result;
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md" dir="rtl">
        <DialogHeader>
          <DialogTitle>تسديد مستحقات الفني</DialogTitle>
          <DialogDescription>{technicianName} • {unpaidPurchases.length} فاتورة مستحقة</DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-1 p-3 bg-primary/5 rounded-lg border border-primary/20">
              ...
              {previewAllocation().map((a, i) => (
                <div key={i} className="flex justify-between text-sm">
                  <span>{a.project} - فاتورة: {a.invoice}</span>
