...
import { ArrowRight, Phone, Mail, Wrench, Briefcase, DollarSign, Calendar, Plus, Trash2, Filter, X, Wallet, TrendingUp, CreditCard, Printer, Landmark, Banknote, Pencil } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
const TechnicianDetail = () => {
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(`
          ...
          project_phases:phase_id(id, name, phase_number, reference_number, treasury_id,
            treasuries:treasury_id(id, name, parent_id)
          )
        `)
        .eq("technician_id", id)
        .eq("is_return", false)
  ...
  const deletePaymentMutation = useMutation({
    mutationFn: async ({ paymentId, type }: { paymentId: string; type: "expense" | "purchase" }) => {
      if (type === "expense") {
        // Delete related treasury transactions first
        await supabase.from("treasury_transactions").delete().eq("reference_id", id).eq("reference_type", "technician_payment");
  ...
  const editPaymentMutation = useMutation({
    mutationFn: async (data: NonNullable<typeof editingPayment>) => {
      const amount = parseFloat(data.amount);
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["treasuries"] });
      queryClient.invalidateQueries({ queryKey: ["treasury_transactions"] });
      toast.success("تم تعديل المدفوعة بنجاح");
  ...
  const treasurySummary = useMemo(() => {
    ...
    const buildPhaseLabel = (phase?: {
      ...
      const sequence = phase.reference_number?.trim()
        || (phase.phase_number ? `مرحلة ${phase.phase_number}` : "");
    ...
    const ensureSummary = (tKey: string, tName: string) => {
      if (!summary[tKey]) {
        summary[tKey] = { treasuryId: tKey, treasuryName: tName, projects: {}, totalDeserved: 0, totalWithdrawn: 0, totalRemaining: 0 };
      }
    };
    const ensureProject = (tKey: string, pId: string, pName: string) => {
      if (!summary[tKey].projects[pId]) {
        summary[tKey].projects[pId] = { id: pId, name: pName, phases: [], deserved: 0, withdrawn: 0, remaining: 0 };
      }
    };
    const addPhaseToProject = (tKey: string, pId: string, phaseLabel: string | null) => {
      if (!phaseLabel) return;
      const project = summary[tKey]?.projects[pId];
      if (!project || project.phases.includes(phaseLabel)) return;
      project.phases.push(phaseLabel);
    };
    const findTreasuryKeyByProject = (projectId: string) => {
    ...
    progressRecords?.forEach((r) => {
      const projectId = r.project_items?.projects?.id || noProjectKey;
      const projectName = r.project_items?.projects?.name || "بدون مشروع";
      ...
      ensureSummary(treasuryKey, treasuryName);
      ensureProject(treasuryKey, projectId, projectName);
      addPhaseToProject(treasuryKey, projectId, buildPhaseLabel(r.project_items?.project_phases));
    ...
    technicianPurchases?.forEach((purchase) => {
      const projectId = (purchase as any).projects?.id || noProjectKey;
      const projectName = (purchase as any).projects?.name || "بدون مشروع";
      const phase = (purchase as any).project_phases;
      ...
      const purchaseTreasuryName = (purchase as any).project_phases?.treasuries?.name || "بدون خزينة";

      if (purchaseTreasuryId) {
        treasuryKey = purchaseTreasuryId;
      } else {
        treasuryKey = findTreasuryKeyByProject(projectId) || noTreasuryKey;
      }

      ensureSummary(
        treasuryKey,
        treasuryKey === noTreasuryKey
          ? "بدون خزينة"
          : summary[treasuryKey]?.treasuryName || purchaseTreasuryName
      );
      ensureProject(treasuryKey, projectId, projectName);
      addPhaseToProject(treasuryKey, projectId, phaseLabel);
    ...
    expenses?.forEach((exp) => {
      ...
      const projectId = exp.project?.id || null;
      const projectName = exp.project?.name || "بدون مشروع";

      if (projectId) {
        const treasuryKey = findTreasuryKeyByProject(projectId) || noTreasuryKey;
        ensureSummary(
          treasuryKey,
          treasuryKey === noTreasuryKey ? "بدون خزينة" : summary[treasuryKey]?.treasuryName || "بدون خزينة"
        );
        ensureProject(treasuryKey, projectId, projectName);
        summary[treasuryKey].projects[projectId].withdrawn += amount;
        summary[treasuryKey].totalWithdrawn += amount;
        return;
      }

      const eligibleProjects = Object.values(summary).flatMap((treasury) =>
        Object.values(treasury.projects)
          .filter((project) => project.id !== noProjectKey)
          .map((project) => ({
            treasuryId: treasury.treasuryId,
            projectId: project.id,
            remaining: roundAmount(project.deserved - project.withdrawn),
          }))
          .filter((project) => project.remaining > 0)
      );

      const totalRemainingPool = eligibleProjects.reduce((sum, project) => sum + project.remaining, 0);

      if (eligibleProjects.length > 0 && totalRemainingPool > 0) {
        let distributedAmount = 0;

        eligibleProjects.forEach((project, index) => {
          const share = index === eligibleProjects.length - 1
            ? roundAmount(amount - distributedAmount)
            : roundAmount((project.remaining / totalRemainingPool) * amount);

          summary[project.treasuryId].projects[project.projectId].withdrawn += share;
          summary[project.treasuryId].totalWithdrawn += share;
          distributedAmount += share;
        });
        return;
      }

      ensureSummary(noTreasuryKey, "بدون خزينة");
      ensureProject(noTreasuryKey, noProjectKey, "بدون مشروع");
      summary[noTreasuryKey].projects[noProjectKey].withdrawn += amount;
      summary[noTreasuryKey].totalWithdrawn += amount;
    });

    // Calculate remaining
    Object.values(summary).forEach((t) => {
      Object.values(t.projects).forEach((p) => {
        p.deserved = roundAmount(p.deserved);
        p.withdrawn = roundAmount(p.withdrawn);
        p.remaining = roundAmount(p.deserved - p.withdrawn);
      });
      t.totalDeserved = roundAmount(t.totalDeserved);
      t.totalWithdrawn = roundAmount(t.totalWithdrawn);
      t.totalRemaining = roundAmount(t.totalDeserved - t.totalWithdrawn);
    });
    ...
    <div className="space-y-6" dir="rtl">
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent dir="rtl">
                              ...
                              <AlertDialogFooter>
                                ...
                                <AlertDialogAction
                                  onClick={() => deletePaymentMutation.mutate({ paymentId: row.id, type: row.type })}
                                >
                                  حذف
                                </AlertDialogAction>
                              </AlertDialogFooter>
      ...
      <Dialog open={!!editingPayment} onOpenChange={(open) => !open && setEditingPayment(null)}>
        <DialogContent dir="rtl" className="max-w-md">
          <DialogHeader>
            <DialogTitle>تعديل المدفوعة</DialogTitle>
          </DialogHeader>
          {editingPayment && (
            <div className="space-y-4">
              <div className="space-y-2">
              ...
              <div className="flex gap-2 justify-end">
                ...
                <Button
                  onClick={() => editPaymentMutation.mutate(editingPayment)}
                  disabled={editPaymentMutation.isPending}
                >
                  {editPaymentMutation.isPending ? "جاري الحفظ..." : "حفظ التعديلات"}
                </Button>
              </div>
            </div>
          )}
