...
const TechnicianDetail = () => {
  ...
  // Fetch treasuries for withdrawal
  ...
  const childTreasuries = allTreasuries.filter(c => (c as any).parent_id === withdrawalForm.parentTreasuryId);

  // Withdrawal mutation
  const withdrawalMutation = useMutation({
    mutationFn: async (data: typeof withdrawalForm) => {
      const amount = parseFloat(data.amount);
      const defaultDesc = `سحب ${data.paymentMethod === "transfer" ? "تحويل" : "نقدي"} - ${technician?.name}`;
  ...
  const handleWithdrawal = () => {
    ...
    if (withdrawalForm.treasuryId) {
      const selectedTreasury = allTreasuries.find(t => t.id === withdrawalForm.treasuryId);
      if (selectedTreasury && parseFloat(withdrawalForm.amount) > (selectedTreasury.balance || 0)) {
        toast.error(`رصيد الخزينة غير كافٍ. المتاح: ${formatCurrencyLYD(selectedTreasury.balance || 0)}`);
        return;
      }
    }
  ...
  const printPaymentsStatement = (dateFrom: string, dateTo: string) => {
    ...
    const rows = filtered.map((e, i) => `
      <tr>
        ...
        <td style="text-align:center">${e.payment_method === "transfer" ? "تحويل" : "كاش"}</td>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(v: "cash" | "transfer") => setWithdrawalForm({ ...withdrawalForm, paymentMethod: v })}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="cash">
                            <span className="flex items-center gap-1"><Banknote className="h-3 w-3" /> كاش</span>
                          </SelectItem>
                          <SelectItem value="transfer">
                            <span className="flex items-center gap-1"><Landmark className="h-3 w-3" /> تحويل</span>
                          </SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-2">
                        ...
                        <Select
                          value={withdrawalForm.parentTreasuryId}
                          onValueChange={(v) => setWithdrawalForm({ ...withdrawalForm, parentTreasuryId: v, treasuryId: "" })}
                        >
                          <SelectTrigger>
                      ...
                      <div className="space-y-2">
                        <Label>الفرع</Label>
                        <Select
                          value={withdrawalForm.treasuryId}
                          onValueChange={(v) => setWithdrawalForm({ ...withdrawalForm, treasuryId: v })}
                          disabled={!withdrawalForm.parentTreasuryId}
                        >
                          <SelectTrigger>
              ...
              <TableBody>
                {expenses.map((expense) => (
                  <TableRow key={expense.id}>
                    ...
                    <TableCell>
                      <Badge variant="secondary">
                        {expense.payment_method === "transfer" ? "تحويل" : "كاش"}
                      </Badge>
                    </TableCell>
