...
export default function InvoiceControl() {
  ...
  // Fetch client payment allocations for purchases
  const { data: clientAllocations = [] } = useQuery({
    queryKey: ["client-allocations-for-purchases"],
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  // Group client allocations by purchase reference_id
  const clientPaidByPurchase = clientAllocations.reduce((acc, a) => {
    acc[a.reference_id] = (acc[a.reference_id] || 0) + Number(a.amount);
    return acc;
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>دفع الزبون</TableHead>
                  <TableHead>المدفوع للمورد</TableHead>
                  <TableHead>المتبقي للمورد</TableHead>
                  <TableHead>العمولة</TableHead>
                  <TableHead>الخزينة</TableHead>
                  <TableHead>حالة المورد</TableHead>
              ...
              <TableBody>
                {filtered.map((p) => {
                  ...
                  const clientPaid = clientPaidByPurchase[p.id] || 0;
                  ...
                  return (
                    <TableRow key={p.id} className={p.status === "due" ? "bg-destructive/5" : ""}>
                      ...
                      <TableCell>
                        {clientPaid > 0 ? (
                          <Badge variant={clientPaid >= Number(p.total_amount) ? "default" : "secondary"} className="flex items-center gap-1 w-fit">
                            <CheckCircle className="h-3 w-3" />
                            {formatCurrencyLYD(clientPaid)}
                          </Badge>
                        ) : (
                          <Badge variant="outline" className="text-muted-foreground w-fit">لم يدفع</Badge>
                        )}
                      </TableCell>
                      <TableCell className="text-green-600 font-medium">{formatCurrencyLYD(Number(p.paid_amount || 0))}</TableCell>
